...
<div class="wrap">
 ...
 <div id="theme_tab_wrap">
  <ul id="theme_tab" class="cf">
   ...
   <li><a href="#tab-content3"><?php _e('Header Banner Setup', 'tcd-w');  ?></a></li>
   <li><a href="#tab-content4"><?php _e('Single Page Banner Setup', 'tcd-w');  ?></a></li>
   <li><a href="#tab-content5"><?php _e('Adsense Widget', 'tcd-w');  ?></a></li>
 ...
 <div id="tab-panel">
  ...
  <div id="tab-content1">
   ...
   <div class="theme_option_field cf">
    ...
    <p color="color_scheme" id="default_color"><?php _e('Default color', 'tcd-w');  ?> ：E41858</p>
   </div>
   ...
   <div class="theme_option_field cf">
    ...
    <p color="color_scheme" id="default_color2"><?php _e('Default color', 'tcd-w');  ?> ：0077B3</p>
   </div>
  ...
  <p class="tab_desc"><?php _e('Adsense will be displayed at random in front page.<br />Don\'t forget to set the Adsense banner Widget from widget page.', 'tcd-w');  ?></p>
   ...
   <div class="theme_option_field cf">
    <div class="theme_option_input">
     <div class="sub_box">
      ...
      <p><?php _e('If you are using google adsense, enter all code below.', 'tcd-w');  ?></p>
      <div class="theme_option_input">
       <textarea id="dp_options[side_ad<?php echo $i; ?>]" class="large-text" cols="50" rows="10" name="dp_options[side_ad<?php echo $i; ?>]"><?php echo esc_textarea( $options['side_ad'.$i] ); ?></textarea>
      </div>
     </div>
     <p><?php _e('If you are not using google adsense, you can register your banner image and affiliate code individually.', 'tcd-w');  ?></p>
     <div class="sub_box cf" style="margin:0 0 10px 0;">
      ...
      <div class="image_box cf">
       <div class="upload_banner_button_area">
        <div class="hide"><input type="text" size="36" name="dp_options[side_ad_image<?php echo $i; ?>]" value="<?php esc_attr_e( $options['side_ad_image'.$i] ); ?>" /></div>
        <input type="file" name="side_ad_image_file_<?php echo $i?>" id="side_ad_image_file_<?php echo $i?>" />
        <input type="submit" class="button-primary" value="<?php echo __( 'Save Image', 'tcd-w' ); ?>" />
       </div>
       <?php if($options['side_ad_image'.$i]) { ?>
        <div class="uploaded_banner_image">
         <img src="<?php esc_attr_e( $options['side_ad_image'.$i] ); ?>" alt="" title="" />
        </div>
        <?php if(dp_is_uploaded_img($options['side_ad_image'.$i])): ?>
        <div class="delete_uploaded_banner_image">
         <a href="<?php echo wp_nonce_url(admin_url('themes.php?page=theme_options'), 'dp_delete_side_ad_'.$i) ?>" class="button" onclick="if(!confirm('<?php _e('Are you sure to delete this image?', 'tcd-w'); ?>')) return false;"><?php _e('Delete Image', 'tcd-w'); ?></a>
        </div>
       <?php endif; ?>
       <?php }; ?>
      </div>
     ...
     <div class="sub_box">
      <h4><?php _e('Register affiliate code', 'tcd-w');  ?></h4>
      <div class="theme_option_input">
       <input id="dp_options[side_ad_url<?php echo $i; ?>]" class="regular-text" type="text" name="dp_options[side_ad_url<?php echo $i; ?>]" value="<?php esc_attr_e( $options['side_ad_url'.$i] ); ?>" />
      </div>
     </div>
    </div>
   </div>
...
function theme_options_validate( $input ) {
 ...
 //広告ウィジェット１画像の登録
 for($i = 1; $i <= 3; $i++){
	 if(isset($_FILES['side_ad_image_file_'.$i])){
		 //画像のアップロードに問題はないか
		 if($_FILES['side_ad_image_file_'.$i]['error'] === 0){
			 $name = sanitize_file_name($_FILES['side_ad_image_file_'.$i]['name']);
			 //ファイル形式をチェック
			 if(!preg_match("/\.(png|jpe?g|gif)$/i", $name)){
				 add_settings_error('design_plus_options', 'dp_uploader', sprintf(__('You uploaded %s but allowed file format is PNG, GIF and JPG.', 'tcd-w'), $name), 'error');
			 }else{
				...
				if(
					(
						...
						&&
					move_uploaded_file($_FILES['side_ad_image_file_'.$i]['tmp_name'], dp_logo_basedir().DIRECTORY_SEPARATOR.$name)
				){
		 ...
		 }elseif($_FILES['side_ad_image_file_'.$i]['error'] !== UPLOAD_ERR_NO_FILE){
			 add_settings_error('default', 'dp_uploader', _dp_get_upload_err_msg($_FILES['side_ad_image_file_'.$i]['error']), 'error');
			 continue;
		 }
	 }
 }	 
